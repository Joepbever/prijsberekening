...
Partial Class page
    ...
    Public Sub btnSubmit_Click(sender As Object, e As EventArgs) Handles btnSubscribe.Click
        ...
        If LI.dt.Rows.Count > 0 Then
            ...
            sBericht = sBericht.Replace("[email]", txtEmailMaster.Text)
            ...
            If LI.dt.Rows.Count > 0 Then
                ...
                sBericht = sBericht.Replace("[name]", txtNameMaster.Text)
                sBericht = sBericht.Replace("[email]", txtEmailMaster.Text)
                ...
                Dim sTemplate As String = M.sHtml("~/EmailTemplates/newMail.aspx")
                sTemplate = sTemplate.Replace("[subject]", sOnderwerp)
                sTemplate = sTemplate.Replace("[body]", sBericht)

                iMailID = M.iscMail(0, BP.iPartijIDBeheerder, "versturen", sType, "", sVan, sNaar, sCC, sBCC, sOnderwerp, sTemplate, sBijlagen, "", sInfo, Now)

                Dim cCookie As HttpCookie = Request.Cookies("Follow")

                Dim sSessie As Sessie = JsonConvert.DeserializeObject(Of Sessies)(cCookie.Value).seSessies(0)

                sSessie.save(BP.iPartijIDBeheerder, txtEmailMaster.Text)
